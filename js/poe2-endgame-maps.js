...
const FormatController = {
  ...
  updateUI() {
    ...
    if (DOM.btnFormatText) {
      DOM.btnFormatText.innerText = labels[mode] || '日＋英';
    }
    ...
    document.body.classList.remove(
      'lang-ja-only', 'lang-en-only', 'lang-both', 'lang-ja-main', 'lang-en-main',
...
const CELL_RENDERERS = {
  ...
  mark: (td, item, def) => {
    ...
    Object.entries(MARK_DEFINITIONS).forEach(([markKey, markDef]) => {
      container.append(TableRenderer.createMarkButton(item.id, markKey, item.user.marks.has(markKey), markDef));
    });
    td.append(container);
  },
  custom: (td, item, def) => {
...
const TableRenderer = {
  ...
  createResistCell(item, td) {
    ...
    const wrapper = document.createElement('div');
    wrapper.className = 'cell-resist-wrapper';
...
const ColumnRenderer = {
  updateButtonUI() {
    ...
    Object.entries(AppState.user.columns).forEach(([key, isVisible]) => {
      ...
      if (menuItem) {
        menuItem.classList.toggle('is-checked', isVisible);
      }
